...
## 📢 업데이트 사항 (26.05.02)
> **🚨 중요:** 코드 구조가 완전히 변경되었으므로, **꼭 파일을 새로 다운로드**하여 사용하시기 바랍니다.

- **킥보드 탐지 기능 추가**: 전용 모델(`PM-Tracker.pt`)을 통한 정밀 탐지 구현
- **코드 모듈화**: 유지보수와 가독성을 위해 기능을 파일별로 분리
- **변수 이름 최적화**: 클린 코드를 위해 직관적인 변수명으로 전면 수정
- **환경 설정 분리**: 상수를 `config.py`로 모듈화하여 설정 변경을 용이하게 개선
